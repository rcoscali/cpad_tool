...
#define fopen_s(fp, f, m)  *(fp) = fopen(f, m)
...
class xstring : public string
	...
	public:
		...
		xstring& operator =(const char* t) { string::operator =(t); return *this; }
		xstring& operator =(const string& t) { string::operator =(t); return *this; }
		xstring& operator =(const xstring& t) { string::operator =(t.c_str()); return *this; }
...
inline int _vscprintf(const char *format, va_list argptr)
	...
	inline string Char2Base64(const char *Z,size_t s)
		{
		DWORD da = 0;
                da = Base64encode_len(s);
		unique_ptr<char> out(new char[da]);
                Base64encode(out.get(), Z, s);
		return out.get();
		}
	...
	inline void Base64ToChar(const char *Z,size_t ,BXML& out)
		{
		DWORD dw = 0;
                dw = Base64decode_len(Z);
		out.Resize(dw);
                Base64decode(out.p(), Z);
		}
...
*/
	...
	// Serialization
	inline string XMLCData::Serialize(bool) const
		{
	...
	// Serialization
	inline string XMLDocType::Serialize(bool) const
		{
	...
	// Serialization
	inline string XMLComment::Serialize(bool) const
		{
		return Format("<!--%s-->",v.c_str());
		}
...
//			operator=(from.Serialize().c_str());
	...
	inline string XMLElement::Serialize(bool NoEnc,size_t deep) const
		{
		string v;
	...
	inline void XMLElement::Serialize(string& v,bool NoEnc,size_t deep) const
		{
		string padd;
...
//					XMLElement ne;
	...
	// ser
	inline string XML::Serialize(bool NoEnc) const
		{
		string v;
		v.reserve(100000);
