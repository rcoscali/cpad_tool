# 1 "Node.cc"
# 1 "/usr/local/src2/Synopsys/build-toolchain-releases/cpad_tool//"
# 1 "<built-in>"
# 1 "<command-line>"
...
namespace cpad
{
  ...
  {
  public:
    Node(string, int, cpad::Edge const&, cpad::Edge const&);
    Node(string);
    Node(Node const&);
    virtual ~Node();
    ...
    void add_default_edge(Edge const&);
    void add_fallback_edge(Edge const&);
...
::cpad::Node::Node(string name,
  ...
  : m_name(name),
    ...
    _has_edge_fb(true)
{
  m_edges = make_pair(edge_d, edge_fb);
  _has_edge_d = true;
  _has_edge_fb = true;
}

::cpad::Node::Node(string name)
  : Node(name, 0, cpad::Edge(), cpad::Edge())
{
...
::cpad::Node::operator = (::cpad::Node const&a_node_to_affect)
{
  ...
  m_edges = a_node_to_affect.m_edges;
...
{
  if (!_has_add_value)
    throw new ::std::logic_error("Node don't have an add_value")
  return m_add_value;
}
...
{
  if (_has_add_value)
    throw new ::std::logic_error("Node already have add value");
  m_add_value = add_value;
  _has_add_value = true;
}
...
{
  if (! (has_an_edge()))
    throw new std::logic_error("This node has no Edge")
  return m_edges;
}

void
::cpad::Node::add_default_edge(::cpad::Edge &an_edge)
{
  m_edges->first = an_edge;
}

void
::cpad::Node::add_fallback_edge(::cpad::Edge &an_edge)
{
  m_edge->second = an_edge;
}
