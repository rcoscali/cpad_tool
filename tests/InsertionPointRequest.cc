...
public:
  ...
  InsertionPointRequestTests()
  {
    buffer = new char[128];
    bzero(buffer, 128);
    iprh1 = new ::cpad::insns::InsertionPointRequestHelper("foobar.c", "foo", FUNCTION_ENTRY_BLOCK);
...
{
  ::cpad::insns::InsertionPointRequestHelper iprh("foobar.c", "foo", FUNCTION_BEFORE_CALL);
  EXPECT_STREQ(iprh.cunit_name().c_str(), "foobar.c");
  EXPECT_STREQ(iprh.cfun_name().c_str(), "foo");
  EXPECT_EQ(iprh.location(), FUNCTION_BEFORE_CALL);
}
...
TEST_F(InsertionPointRequestTests, DeserializeConstructor)
{
  ...
  EXPECT_EQ(iprh1->location(), FUNCTION_BEFORE_CALL);
}
...
TEST_F(InsertionPointRequestTests, DeserializeConstructorFromEmptyString)
{
  ...
  EXPECT_EQ(iprh2.location(), FUNCTION_ENTRY_BLOCK);
}
...
{
  ::cpad::insns::InsertionPointRequestHelper iprh("foobar.c", "foo", , FUNCTION_EXIT_BLOCK);
